...
#define rs(x) (node[x].r)
    ...
    void pushdown(int id) {
        node.push_back(node[ls(id)]);
        ls(id) = node.size() - 1;
        node.push_back(node[rs(id)]);
        rs(id) = node.size() - 1;
        apply(ls(id), node[id].tag);
        apply(rs(id), node[id].tag);
        node[id].tag = Tag();
    }
    ...
    int rangeUpdate(int lst, int l, int r, const int &x, const int &y, const Tag &dx) {
        ...
        if(x <= l && r <= y) {
            ...
            pushdown(id);
            if(x <= mid) {
                ls(id) = rangeUpdate(ls(id), l, mid, x, y, dx);
            }
            if(y > mid) {
                rs(id) = rangeUpdate(rs(id), mid + 1, r, x, y, dx);
            }
            node[id].info = node[ls(id)].info + node[rs(id)].info;
        }
        return id;
