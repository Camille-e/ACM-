...
template<class T>bool pointOnSegment(Pt p, Lt l) {
    return sign(cross(p, l.a, l.b)) == 0 &&
           min(l.a.x, l.b.x) - EPS <= p.x && p.x <= max(l.a.x, l.b.x) + EPS &&
           min(l.a.y, l.b.y) - EPS <= p.y && p.y <= max(l.a.y, l.b.y) + EPS;
}
...
template<class T> tuple<int, Pt, Pt> segmentIntersection(Lt l1, Lt l2) { // 两线段是否相交及交点 segmentIntersection
    // 0 代表不相交; 1 代表普通相交; 2 代表重叠（交于两个点）; 3 代表相交于端点.s
    auto [s1, e1] = l1;
    auto [s2, e2] = l2;
...
vector<Point<T>> staticConvexHull(vector<Point<T>> A, int flag = 1) {
    ...
    for (auto p : A) {
        while ((int)lower.size() >= 2 &&
               sign(cross(lower.back(), p, lower[(int)lower.size() - 2])) < flag) {
            lower.pop_back();
        }
        lower.push_back(p);
    }
    for (int i = n - 1; i >= 0; --i) {
        ...
        while ((int)upper.size() >= 2 &&
               sign(cross(upper.back(), p, upper[(int)upper.size() - 2])) < flag) {
            upper.pop_back();
        }
        upper.push_back(p);
    }
